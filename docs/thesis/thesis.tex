...
\label{chap:intro}
Deep neural networks have achieved widespread success across modern domains, from language models to medical diagnostics and autonomous robotics. However, standard training relies on fixed, independent, and identically distributed (i.i.d.) datasets. True intelligent systems must instead possess the capacity for continual learning, acquiring knowledge over time from evolving data streams without retraining from scratch. This capability is essential for dynamic real-world environments and serves as a key step toward mimicking biological intelligence. Yet, transitioning neural networks from static to continual learning reveals a major vulnerability: when adapted to new tasks, models systematically overwrite prior knowledge, encountering the stability-plasticity dilemma.
...
\end{equation}
The mini-batch approximation introduces stochastic fluctuations into the parameter trajectory. Rather than behaving as pure numerical noise, these fluctuations act similarly to thermal fluctuations in a physical system. They help the optimiser overcome shallow barriers, escape brittle saddle points and sharp local minima, and guide the parameter configuration into broad, robust basins that offer superior generalisation.
...
Evaluating fixed-capacity networks across these dynamical regimes typically requires structural constraints to preserve historical representations without storing past data. While standard approaches apply algorithmic constraints strictly post-initialisation, their effectiveness is fundamentally bounded by how representations occupy space within the weight space. To investigate how representation geometry governs sequential learning, we focus on Gradient Projection Memory (GPM), a subspace-constrained framework designed to preserve historical task knowledge through orthogonal parameter updates.
...
\begin{equation}
	p(x) \sim \frac{c_\alpha \sigma^\alpha}{|x|^{1+\alpha}} \quad \text{as } |x| \to \infty
\end{equation}
	...
	\begin{tabular}[b]{cc}
		\begin{tabular}[b]{c}
			\begin{overpic}[width=.3\textwidth]{imgs/DOS_alpha_120.pdf}
				...
				\put (60,20) {\includegraphics[width=.15\textwidth]{imgs/DOS_alpha_120_inset.pdf}}
				\put (60,-50) {\includegraphics[width=.15\textwidth]{imgs/DOS_alpha_200_inset.pdf}}
			\end{overpic}\\
...
\end{equation}
where $f(|z|)$ is a monotonically increasing test function that penalizes near-zero eigenvalues and rewards larger moduli capable of sustaining gradients across depth $L$. In Gaussian networks ($\alpha = 2$), this average is strictly maximized at the isolated critical point $D_w = 1$. 
