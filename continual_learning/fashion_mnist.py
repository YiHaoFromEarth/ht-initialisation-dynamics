...
f_train = datasets.FashionMNIST(
    "./data", train=True, download=True, transform=transforms.ToTensor()
)
f_test = datasets.FashionMNIST(
...
for seed in seeds:
    ...
    for alpha, g in itertools.product(alpha_list, g_list):
        ...
        output_dir = Path(
            f"./continual_learning/sweep_fashion_gpm{GPM_THRESHOLD}_fc{depth + 1}/{run_name}"
        )
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        for t_idx in range(num_tasks):
            ...
            train_loader = DataLoader(
                TensorDataset(curr_train_imgs, curr_train_lbls),
                batch_size=batch_size,
                shuffle=True,
            )
            ...
            gpm_feature_bases, task_ranks = update_GPM_bases(
                model, svd_samples, GPM_THRESHOLD, gpm_feature_bases
            )
